...
| 插件架构 / Microkernel | 机制同源、意图相反 | 端口=内核主动定义的抽象（防御，隔离已知外部技术）；插件点=宿主被动接纳的扩展（开放，接纳未知未来功能）。结构上插件点≈ driving port 的运行时动态装配变体；Microkernel（VSCode/Jenkins 式）是插件意图的极端化 |
| Screaming / Package by Feature | 正交维度 | 管目录按业务特性而非按层组织；与内外边界可叠加，不冲突 |
